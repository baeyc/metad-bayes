...
dline <- data.frame(flux=rep(c("A","B"),each=6),comp=rep(c("M vs part","M vs Rule"),6),
                    variable=rep(c("d'","meta-d","Mratio"),2),value=rep(c(0,0,1),2))
...
boxplotall <- dplotall %>% filter(type=="Posterior mean") %>%
  ...
  geom_hline(data=dline,aes(yintercept=value),col="red")
...
Mratio_plot <- comp_posteriors_flux("mu_logMratio") + xlab(expression(mu[M]))
ggsave(paste0("figures/type2_",studyname,"_postdist_mu_logMratio_flux_A_B.pdf"),Mratio_plot,width = 5, height = 5)
mu_d1_plot <- comp_posteriors_flux("mu_d1") + xlab(expression(mu[d]))
ggsave(paste0("figures/type2_",studyname,"_postdist_mu_d1_flux_A_B.pdf"),mu_d1_plot,width = 5, height = 5)


# Posterior distribution of the difference between languages
post_diff_mu_d <- mu_d1_plot$data %>% filter(flux=="A") %>% select(value) - 
  mu_d1_plot$data %>% filter(flux=="B") %>% select(value)
post_diff_mu_d %<>% cbind(mu_d1_plot$data %>% filter(flux=="A") %>% select(comp))

dci <- lapply(unique(post_diff_mu_d$comp), FUN = function(i){
  temp <- post_diff_mu_d$value[post_diff_mu_d$comp == i]
  ci <- hdi(temp, ci=0.95)
  return(data.frame(ci_low=ci["CI_low"],ci_high=ci["CI_high"],postmean=mean(temp)))
})

dci <- do.call(rbind,dci)
dci$comp <- unique(post_diff_mratio$comp)


p <- ggplot(data=post_diff_mu_d,aes(value)) +
  geom_histogram(position = "identity", binwidth = 0.03, alpha = 0.5) +
  ylab("Sample count") + scale_fill_grey(start=0.45,end=0.7) + facet_wrap(~comp, nrow = 2) +
  geom_segment(data=dci,aes(x=CI_low,xend=CI_high,y=50,yend=50),col="white",size=2) + geom_vline(xintercept = 0, col="red")
ggsave(paste0("figures/type2_",studyname,"_postdist_diff_mu_d1_flux_A_B.pdf"),p,width = 4, height = 5)


# M ratio
post_diff_mratio <- Mratio_plot$data %>% filter(flux=="A") %>% select(value) - 
  Mratio_plot$data %>% filter(flux=="B") %>% select(value)
post_diff_mratio %<>% cbind(Mratio_plot$data %>% filter(flux=="A") %>% select(comp))

dci <- lapply(unique(post_diff_mratio$comp), FUN = function(i){
  temp <- post_diff_mratio$value[post_diff_mratio$comp == i]
  ci <- hdi(temp, ci=0.95)
  return(data.frame(ci_low=ci["CI_low"],ci_high=ci["CI_high"],postmean=mean(temp)))
})
...
p <- ggplot(data=post_diff_mratio,aes(value)) +
  geom_histogram(position = "identity", binwidth = 0.03, alpha = 0.5) +
  ylab("Sample count") + scale_fill_grey(start=0.45,end=0.7) + facet_wrap(~comp, nrow = 2) +
  geom_segment(data=dci,aes(x=CI_low,xend=CI_high,y=50,yend=50),col="white",size=2) + geom_vline(xintercept = 0, col="red")
p
ggsave(paste0("figures/type2_",studyname,"_postdist_diff_mratio_flux_A_B.pdf"),p,width = 4, height = 5)
